...
3. 이 저장소에는 보안 규칙/인덱스 정의가 이미 포함되어 있습니다. Firebase CLI로 배포합니다.
   ...
   `firestore.indexes.json`에는 예약 날짜범위·인보이스 코디네이터 스코프·환자검색 등에 쓰이는 **복합 인덱스 21개**가 정의되어 있습니다. 이 인덱스를 먼저 배포하지 않으면 해당 쿼리들이 런타임에 "The query requires an index" 에러로 실패합니다(에러 메시지에 포함된 콘솔 링크로도 개별 생성 가능하지만, 위 명령으로 한 번에 배포하는 편이 안전합니다).

### Cloud Storage 운영 접근 정책

예약 사진(`reservationFiles/{reservationDocId}/...`)은 민감정보이므로 인증된 활성 직원만 접근할 수 있습니다. 현재 운영 정책은 다음과 같습니다.

- 모든 활성 직원은 모든 예약 사진을 읽고 이미지 파일을 업로드·수정·삭제할 수 있습니다.
- **예약별·담당자별 접근 제한 없음**이 현재 의도된 정책입니다.
- 비인증 사용자와 비활성 직원은 접근할 수 없으며, 이미지가 아닌 파일과 10MB 이상 업로드는 차단됩니다.
- 담당 예약 또는 업로더 단위 제한이 필요해지면 Storage 메타데이터와 예약 권한을 연동하는 별도 정책 변경이 필요합니다.
