          ...
          <div className="flex h-full flex-col justify-between lg:gap-2">
            <div className="grid grid-cols-[minmax(88px,1fr)_auto_minmax(88px,1fr)_104px] items-center gap-1.5 lg:grid-cols-[minmax(130px,1fr)_auto_minmax(130px,1fr)_minmax(120px,0.8fr)_minmax(180px,1.2fr)_auto] lg:gap-2">
              ...
              <button
                onClick={() => load()}
                disabled={loading}
                className="hidden h-10 shrink-0 rounded-[18px] bg-[linear-gradient(135deg,#77dfd1_0%,#40c5b3_50%,#0f9b8e_100%)] px-5 text-[11px] font-bold text-white shadow-[0_10px_24px_rgba(15,143,131,0.14)] transition active:scale-95 disabled:opacity-60 lg:block lg:text-xs"
              >
                {loading ? "조회 중…" : "조회"}
              </button>
            </div>
          ...
          {loading ? (
            ...
            <>
              {pagedInvoices.map((inv) => (
                <article key={inv.id} className="rounded-[24px] bg-white p-2.5 shadow-[0_10px_28px_rgba(15,23,42,0.05)]">
                  ...
                  <div className="mt-1.5 border-t border-[#edf0f3] pt-1.5">
                    <div className="flex flex-col gap-1.5 lg:flex-row lg:items-center lg:justify-between">
                      ...
                      <div className="flex flex-wrap gap-1.5 lg:justify-end">
                        <button onClick={(e) => { e.stopPropagation(); setSelectedInvoice(inv); }} className="h-6 rounded-[13px] bg-[#e3f2ee] px-2 text-[10px] font-semibold text-[#0f9b8e] transition active:scale-95">보기</button>
                        <button onClick={(e) => handleDelete(inv, e)} className="h-6 rounded-[13px] bg-red-50 px-2 text-[10px] font-semibold text-red-500 transition active:scale-95">삭제</button>
                      </div>
                    </div>
                  </div>
                </article>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 border-t border-[#edf0f3] pt-3 text-xs text-gray-400">
          ...
          {filtered.length > PAGE_SIZE && (
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="h-7 rounded-full bg-[#e3f2ee] px-3 font-semibold text-[#0f9b8e] disabled:opacity-40">다음</button>
            </div>
          )}
