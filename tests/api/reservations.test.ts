...
test("update: staff가 화이트리스트 밖 필드(isDeleted/invoiceDocId/createdByUid)를 보내면 무시된다", async () => {
  __resetStaffCacheForTests();
  const created = await POST(
    makeReq(admin.idToken, "create", {
      ...
      reservation: {
        ...
        reservationDate: "2026-07-10", doctors: [], isDeleted: false,
      },
    })
  );
  ...
  assert.equal(data.hospital, "허용병원");        // 허용 필드는 반영
  assert.equal(data.isDeleted, false);             // 금지 필드는 무시 → 삭제 우회 차단
  assert.notEqual(data.invoiceDocId, "hacked");
  assert.notEqual(data.createdByUid, "spoof");
});

test("update: 서버가 reservation_update 감사로그를 기록한다", async () => {
  __resetStaffCacheForTests();
  const rid = `R-LOG-${Date.now()}`;
  const created = await POST(
    makeReq(admin.idToken, "create", {
      patient: { name: "로그", patientId: `P-LOG-${Date.now()}` },
      reservation: { reservationId: rid, name: "로그", reservationDate: "2026-07-11", doctors: [], isDeleted: false },
    })
  );
  const createdBody = await created.json();
  ...
  await POST(
    makeReq(staff.idToken, "update", {
      ...
      reservationId: rid,
      reservationPatch: { hospital: "로그병원" },
    })
  );
  ...
  assert.equal(logs.docs[0].data().staffUid, staff.uid);
});
