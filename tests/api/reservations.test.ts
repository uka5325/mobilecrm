...
test("update: staff가 화이트리스트 밖 필드(isDeleted/invoiceDocId/createdByUid)를 보내면 400 거부 + DB 불변", async () => {
  ...
  assert.equal(data.hospital, "원본병원");
...
test("update: 정상 필드만이면 200 + reservation_update 감사로그(before/after) 기록", async () => {
  ...
  assert.equal(data.hospital, "새병원");
  assert.equal(data.updatedByUid, staff.uid);
  ...
  assert.equal((log.before as Record<string, unknown>).hospital, "이전병원");
  assert.equal((log.after as Record<string, unknown>).hospital, "새병원");
});
...
test("create: 기존 patientId로 재요청하면 patients가 중복 생성되지 않고 연결만 된다", async () => {
  ...
  assert.equal(firstBody.patientDocId, patientId);
...
test("patients summary: 예약 생성/추가/삭제 시 예약 요약 필드가 정확히 재계산된다", async () => {
  __resetStaffCacheForTests();
  ...
  assert.equal(pat.lastReservationDate, "2026-05-20");
